...
- PDF 路径
- 输出目录
- backend：`auto|mineru|docling|pymupdf|pymupdf-ocr`
...
`auto` 顺序：MinerU → Docling → PyMuPDF 文本层 → PyMuPDF + Tesseract OCR。不得因为 MinerU 不可用或 PDF 没有文本层就停止整个流程。
...
- 扫描版 PDF 使用 PyMuPDF 得到空文本时，应自动回退到 `pymupdf-ocr`；缺少 Tesseract 或中文语言包时再建议安装 MinerU/Docling OCR 后端并重跑。
